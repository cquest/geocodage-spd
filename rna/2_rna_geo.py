...
data_csv = csv.reader(open(sys.argv[1],'r'))
data_geo = csv.writer(open('geo-'+sys.argv[1],'w'))

# chargement de la liste des communes et lat/lon des chef-lieu
...
conn = sqlite3.connect('cache/cache_addok_'+sys.argv[1]+'.db')
...
for et in data_csv:
    if header is None:
        header = et+['longitude','latitude','geo_score','geo_type','geo_adresse','geo_id','geo_ligne','geo_insee']
        ...
        numvoie = numbers.match(et[17]).group(0)
        indrep = et[18]
        typvoie = et[19]
        libvoie = et[20]
        ...
        ligne4N = ('%s %s %s' % (et[28],et[30],et[31])).strip()
        ligne4D = ('%s %s %s %s %s' % (et[26],et[27],et[29],et[30],et[31])).strip()
        # code INSEE de la commune
        depcom = et[22]

        #print('%s|%s|%s|%s' % (depcom,ligne4G,ligne4N,ligne4D))
            ...
            try:
                ...
                if ligne4G.strip() !='':
                    if typvoie == '' and ['CHEF LIEU','CHEF-LIEU','BOURG','LE BOURG','AU BOURG'].count(libvoie)>0:
                        ...
                        print(json.dumps({'action':'manque','id': et[0], 'id2': et[1], 'adr_insee': depcom, 'adr_texte': ligne4G.strip()},sort_keys=True))
                else:
                    stats['vide']+=1
                    ok = ok +1
            except:
                pass
            data_geo.writerow(row)
        else:
            ok = ok +1
            if ['village','town','city'].count(source['properties']['type'])>0:
                source['properties']['type'] = 'municipality'
            stats[source['properties']['type']]+=1
            if 'citycode' not in source['properties'] and source['properties']['id'][:4]=='BANO':
                source['properties']['citycode'] = source['properties']['id'][5:10]
            data_geo.writerow(et+[source['geometry']['coordinates'][0],
